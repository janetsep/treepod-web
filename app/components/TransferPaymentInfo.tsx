...
Email: ${bankDetails.email}
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            <div className="bg-surface-light border p-8 rounded-3xl border-gray-200">
                ...
                <div className="space-y-4 text-sm font-light text-text-sub">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <p className="text-[10px] uppercase tracking-widest font-bold text-text-main/50 mb-1">Banco</p>
                            <p className="font-medium text-text-main">{bankDetails.bank}</p>
                        </div>
                        <div>
                            <p className="text-[10px] uppercase tracking-widest font-bold text-text-main/50 mb-1">Tipo de Cuenta</p>
                            <p className="font-medium text-text-main">{bankDetails.accountType}</p>
                        </div>
                        <div>
                            <p className="text-[10px] uppercase tracking-widest font-bold text-text-main/50 mb-1">N° Cuenta</p>
                            <p className="font-medium text-text-main font-mono">{bankDetails.accountNumber}</p>
                        </div>
                        <div>
                            <p className="text-[10px] uppercase tracking-widest font-bold text-text-main/50 mb-1">RUT</p>
                            <p className="font-medium text-text-main">{bankDetails.rut}</p>
                        </div>
                        <div className="sm:col-span-2">
                            <p className="text-[10px] uppercase tracking-widest font-bold text-text-main/50 mb-1">Titular</p>
                            <p className="font-medium text-text-main">{bankDetails.holder}</p>
                        </div>
                        <div className="sm:col-span-2">
                            <p className="text-[10px] uppercase tracking-widest font-bold text-text-main/50 mb-1">Correo de Confirmación</p>
                            <p className="font-medium text-text-main">{bankDetails.email}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-8 pt-8 border-t border-gray-100">
                    <div className="flex justify-between items-center mb-2">
                        <span className="text-sm font-bold uppercase tracking-widest text-text-sub">Monto a Transferir (50%)</span>
                        <span className="text-2xl font-display font-bold text-primary">${depositAmount.toLocaleString("es-CL")}</span>
                    </div>
                    <p className="text-xs text-right text-text-sub/60 italic">
            ...
            <button
                ...
                disabled={isLoading}
                className="w-full bg-primary hover:bg-primary-dark text-white font-semibold py-5 rounded-full text-base shadow-xl transform hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-4 disabled:opacity-50 disabled:cursor-not-allowed"
            >
                {isLoading ? (
